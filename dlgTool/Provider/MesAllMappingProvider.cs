...
{
    class MesAllMappingProvider : MappingProvider
    {
        private const string MappingPath_ = "mappings";
        private const string MappingFileFormat_ = "{0}_{1}.json";

        private MesAllMappingProvider(Mapping mapping) :
            base(new MappedOpCodeProvider(mapping.OpCodes), new MappedCharacterProvider(mapping.Characters))
        { }
        ...
        public static bool TryGet(Game game, Region region, out MappingProvider? provider)
        {
            ...
            provider = new MesAllMappingProvider(mapping);
            return true;
        }
